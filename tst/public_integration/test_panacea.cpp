...
TEST_CASE("Testing:panacea self entropy", "[integration,panacea]") {
  ...
  WHEN("Testing self entropy term with single point.") {
    ...
    std::unique_ptr<EntropyTerm> self_ent =
        panacea_pi.create(dwrapper.get(), panacea_settings);
...
TEST_CASE("Testing:panacea self entropy shell + initialize",
  ...
  WHEN("Using OneToOne mapping.") {
    ...
    WHEN("Testing self entropy term with single point.") {
      ...
      // be calculating th entropy at the new peak location. If the entropy term
      // did not share the data then these values would be different.
      dwrapper->operator()(0, 0) = 2.0;
  ...
  WHEN("Using Single mapping.") {
    ...
    WHEN("Testing self entropy term with single point.") {
      ...
      // if the descriptors are being changed would prove difficult. E.g. if you
      // are using the median, what would calculate_grad look like?
...
TEST_CASE("Testing:panacea cross entropy single mean",
          "[integration,panacea]") {
...
TEST_CASE("Testing:panacea cross entropy single mean update",
  ...
  std::unique_ptr<EntropyTerm> cross_ent =
      panacea_pi.create(dwrapper_init.get(), panacea_settings);
...
TEST_CASE("Testing:panacea cross entropy single median update",
  ...
  double cross_ent_val_single_pt_before_update =
      cross_ent->compute(dwrapper.get());
  std::cout << __FILE__ << ":" << __LINE__ << std::endl;
  cross_ent->update(dwrapper.get());
  std::cout << __FILE__ << ":" << __LINE__ << std::endl;
  ...
  REQUIRE(cross_ent_val_single_pt_after_update ==
          Approx(cross_ent_val_single_pt_before_update));
  std::cout << __FILE__ << ":" << __LINE__ << std::endl;
...
TEST_CASE("Testing:panacea cross entropy single median weighted",
  ...
  std::unique_ptr<EntropyTerm> cross_ent1 =
      panacea_pi.create(dwrapper_init.get(), panacea_settings1);
  std::unique_ptr<EntropyTerm> cross_ent2 =
      panacea_pi.create(dwrapper_init.get(), panacea_settings2);
