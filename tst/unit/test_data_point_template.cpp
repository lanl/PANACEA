...
TEST_CASE("Testing:data_point_template constructors","[unit,panacea]"){
  WHEN("Using default memory layout") {
    ...
    REQUIRE(data1.arrangement() == Arrangement::PointsAlongRowsDimensionsAlongCols);
    REQUIRE(data1.layout() == MemoryLayout::Default);
    DataPointTemplate<std::vector<std::vector<double>>> data2;
    REQUIRE(data2.arrangement() == Arrangement::PointsAlongRowsDimensionsAlongCols);
    REQUIRE(data2.layout() == MemoryLayout::Default);
    DataPointTemplate<std::vector<double> *> data3;
    REQUIRE(data3.arrangement() == Arrangement::PointsAlongRowsDimensionsAlongCols);
    REQUIRE(data3.layout() == MemoryLayout::Default);
    DataPointTemplate<std::vector<double>> data4;
    REQUIRE(data4.arrangement() == Arrangement::PointsAlongRowsDimensionsAlongCols);
    REQUIRE(data4.layout() == MemoryLayout::Default);
    DataPointTemplate<double ***> data5;
    REQUIRE(data5.arrangement() == Arrangement::PointsAlongRowsDimensionsAlongCols);
...
TEST_CASE("Testing:data template wrapper test with nested vector","[unit,panacea]"){

  std::vector<std::vector<double>> data = test::generateVecData();

  DataPointTemplate<std::vector<std::vector<double>>*> data_template(&data,2, 3);
  auto data_type_index = type_index(typeid(vector<vector<double>> *));
  REQUIRE(data_type_index == std::type_index(data_template.getPointerToRawData().type()));
  ...
  DataPointTemplate<std::vector<std::vector<double>>> data_template2(data,2, 3);
  auto data_type_index2 = type_index(typeid(const std::vector<std::vector<double>> *));
  REQUIRE(data_type_index2 == std::type_index(data_template2.getPointerToRawData().type()));
...
TEST_CASE("Testing:data template wrapper test with nested double ***","[unit,panacea]"){
