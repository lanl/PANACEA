...
TEST_CASE("Testing:normalizer1", "[unit,panacea]") {
  ...
  auto cov_ptr =
      Covariance::create(settings::KernelCorrelation::Correlated,
                         std::move(matrix), std::move(vector), total_num_pts);
...
TEST_CASE("Testing:normalizer write & read", "[unit,panacea]") {
  ...
  auto data_out =
      Normalizer::write(settings::FileType::TXTRestart, fs, &normalizer);
  fs.close();
  ...
  auto data_out2 =
      Normalizer::read(settings::FileType::TXTRestart, fs2, &normalizer2);
  fs2.close();
