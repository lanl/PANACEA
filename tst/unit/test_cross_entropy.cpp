...
TEST_CASE("Testing:cross entropy", "[unit,panacea]") {
  ...
  WHEN("Testing Numerical Decorator centered over kernel") {
    ...
    std::cout << "Numerical cross entropy grad of sample point located at the "
                 "kernel center is: "
  ...
  WHEN("Testing Numerical Decorator to the side of the kernel") {
    ...
    auto analy_grad =
        entropy_term->compute_grad(&dwrapper_sample_max, 0, settings);
    std::cout << "Analytical cross entropy grad of sample point should to the "
                 "side of the kernel center should be negative: "
              << analy_grad.at(0) << std::endl;

    /*auto entropy_term_numer2 =
    std::make_unique<NumericalGrad>(std::move(entropy_term));

    auto numer_grad = entropy_term_numer2->compute_grad(&dwrapper_sample_max, 0,
    settings); std::cout << "Numerical cross entropy grad of sample point
    located to the side of the kernel center is: " << numer_grad.at(0) <<
    std::endl;
