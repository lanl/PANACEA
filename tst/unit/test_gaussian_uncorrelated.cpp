...
TEST_CASE("Testing:creation of gaussian uncorrelated primitive",
          "[unit,panacea]") {
  GaussUncorrelated gauss(test::Test::key(), 0);
  REQUIRE(gauss.getPreFactor() == Approx(0.0));
}

TEST_CASE("Testing:compute of gaussian uncorrelated primitive",
          "[unit,panacea]") {
  ...
  auto gauss_uncorrelated_prim_grp =
  ...
  REQUIRE(gauss_uncorrelated_prim_grp.primitives.at(0)->compute(*dwrapper, 0) ==
          Approx(0.2820947918));
  REQUIRE(gauss_uncorrelated_prim_grp.primitives.at(1)->compute(*dwrapper, 1) ==
          Approx(0.2820947918));

  REQUIRE(gauss_uncorrelated_prim_grp.primitives.at(0)->compute(*dwrapper, 1) ==
          Approx(0.1037768744));
  REQUIRE(gauss_uncorrelated_prim_grp.primitives.at(1)->compute(*dwrapper, 0) ==
          Approx(0.1037768744));
...

  {
    ...
    WHEN("Taking graident with respect to kernel") {
      ...
      const auto grad =
          gauss_uncorrelated_prim_grp.primitives.at(kernel_ind)
              ->compute_grad(*dwrapper, descriptor_ind, eq_settings, setting);
...
TEST_CASE("Testing:compute of gaussian uncorrelated primitive grad",
          "[unit,panacea]") {
  ...
  for (int index = 0; index < numbers1.size(); ++index) {
    ...
    const double val2 = gauss_uncorrelated_prim_grp.primitives.at(kernel_ind)
    ...
    const double val3 = gauss_uncorrelated_prim_grp.primitives.at(kernel_ind)
...
TEST_CASE("Testing:compute of gaussian uncorrelated primitive grad with "
  ...
  for (int index = 0; index < numbers1.size(); ++index) {
    ...
    const std::vector<double> norm_coeffs =
        gauss_uncorrelated_prim_grp.normalizer->getNormalizationCoeffs();
    const double grad_val_numer =
        (val2 - val3) / (raw_desc_data2.at(1).at(0) -
                         raw_desc_data3.at(1).at(0)); // * norm_coeffs.at(0);

    std::cout << "Grad val diff top " << (val2 - val3) << std::endl;
    std::cout << "Grad val diff bottom "
...
TEST_CASE("Testing:gaussian uncorrelated update", "[integration,panacea]") {
  ...
  GIVEN("raw data consisting of one dimension and three points.") {
    std::vector<std::vector<double>> raw_desc_data{{0.0}, {-3.0}, {3.0}};
    ...
    Primitive &primitive =
        *gauss_uncorrelated_prim_grp.primitives.at(primitive_index);
  ...
  GIVEN("raw data consisting of two dimension and three points.") {
    ...
    auto gauss_uncorrelated_prim_grp2 =
        prim_factory.createGroup(*dwrapper, specification2);
    ...
    Primitive &primitive2 =
        *gauss_uncorrelated_prim_grp2.primitives.at(primitive_index);
    const double pre_factor2 = primitive2.getPreFactor();
    // The pre factor 1 should be greater than prefactor 2 because it has fewer
    // dimensions and the determinant of the covariance matrix will be the same
