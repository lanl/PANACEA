...
TEST_CASE("Testing:compute of gaussian uncorrelated primitive",
  ...
  REQUIRE(gauss_uncorrelated_prim_grp.primitives.at(0)->compute(
              dwrapper.get(), 0) == Approx(0.282095));
  REQUIRE(gauss_uncorrelated_prim_grp.primitives.at(1)->compute(
...

  {
    ...
    WHEN("Taking graident with respect to kernel") {
      ...
      const auto grad = gauss_uncorrelated_prim_grp.primitives.at(kernel_ind)
...
TEST_CASE("Testing:compute of gaussian uncorrelated primitive grad",
  ...
  for (int index = 0; index < numbers1.size(); ++index) {
    ...
    grad_wrt_desc = gauss_uncorrelated_prim_grp.primitives.at(kernel_ind)
                        ->compute_grad(dwrapper.get(), descriptor_ind,
