...
TEST_CASE("Testing:covariance large non trival read & write using fileio",
          "[integration,panacea]") {
  ...
  GIVEN("Some large non trivial descriptor wrapper.") {
    ...
    WHEN("The covariance matrix is correlated.") {
      ...
      auto restart_file =
          file_io_factory.create(settings::FileType::TXTRestart);

      restart_file->write(&cov, "test_covariance_correlated_full.restart");

      auto cov_ptr2 =
          Covariance::create(settings::KernelCorrelation::Correlated);
    ...
    WHEN("The covariance matrix is uncorrelated.") {
      ...
      auto cov_ptr2 =
          Covariance::create(settings::KernelCorrelation::Uncorrelated);
...
TEST_CASE("Testing:covariance read & write using fileio",
          "[integration,panacea]") {
