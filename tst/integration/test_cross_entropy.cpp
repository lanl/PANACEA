...
TEST_CASE("Testing:cross entropy", "[integration,panacea]") {
  ...
  std::vector<std::vector<double>> data{{1.0, 3.0}, {2.0, 5.0}, {6.0, 7.0}};
  ...
  GIVEN("A single correlated primitive.") {
    ...
    WHEN("Testing Numerical Decorator centered over kernel") {
      // entropy term needs to be created here because it is moved when
      // decorated
      ...
      std::cout << "Cross entropy should increase when descriptors move away "
                   "from the center of the cross entropy term: "
                << val2 << std::endl;
      ...
      auto analy_grad =
          entropy_term->compute_grad(dwrapper_sample_max, 0, settings);
      std::cout << "Analytical cross entropy grad of sample point should be "
                   "located at the kernel center should be 0.0 it is: "
                << analy_grad.at(0) << std::endl;

      auto entropy_term_numer =
          std::make_unique<NumericalGrad>(std::move(entropy_term));

      auto numer_grad =
          entropy_term_numer->compute_grad(dwrapper_sample_max, 0, settings);
      std::cout << "Numerical cross entropy grad of sample point located at "
                   "the kernel center is: "
                << numer_grad.at(0) << std::endl;

      REQUIRE(numer_grad.at(0) == Approx(analy_grad.at(0)));

      // Now if we use the location of the sample point such that it is a little
      // removed
    }
    ...
    WHEN("Testing Numerical Decorator to the side of the kernel") {
      ...
      REQUIRE(entropy_term != nullptr);
      REQUIRE(entropy_term.get() != nullptr);
      std::vector<std::vector<double>> data_sample_max{{2.8, 5.0}};
      DescriptorWrapper<std::vector<std::vector<double>> *> dwrapper_sample_max(
          &data_sample_max, 1, 2);

      auto analy_grad =
          entropy_term->compute_grad(dwrapper_sample_max, 0, settings);
      std::cout << "Analytical cross entropy grad of sample point should to "
                   "the side of the kernel center should be negative: "
                << analy_grad.at(0) << std::endl;

      auto entropy_term_numer2 =
          std::make_unique<NumericalGrad>(std::move(entropy_term));

      auto numer_grad =
          entropy_term_numer2->compute_grad(dwrapper_sample_max, 0, settings);
      std::cout << "Numerical cross entropy grad of sample point located to "
                   "the side of the kernel center is: "
                << numer_grad.at(0) << std::endl;
