...
class MinMaxDescCache:
    ...
    def __writeCache(self):
        ...
        with open(self.__name_path,"w") as cache_fid:
            ...
            cache_fid.write("Dim1 | Dim2 | Bin Width Dim1 | Bin Width Dim2 |     vmax\n")
            index = 0
            for dim1 in range(0,self.__ndim-1):
                for dim2 in range(1,self.__ndim):
                    if dim2 > dim1:
                        cache_fid.write("%5d  %5d  %15e  %15e  %14e\n" % (dim1,dim2,self.__bin_widths[dim1], self.__bin_widths[dim2], self.__den_val[index]))
                        index = index + 1
        ...
        with open(self.__name_path,"r") as cache_fid:
            ...
            for line in cache_fid:
                if line_num == 0:
                    ...
                    self.__den_val = np.resize(self.__den_val, int((self.__ndim*self.__ndim - self.__ndim)/2.0))
    ...
    def createCache(self,all_descriptors):
        ...
        self.__bin_widths = np.subtract(self.__max_vals.astype(float),self.__min_vals.astype(float))/float(self.__num_bins)
            ...
            for dim2 in range(1,self.__ndim):
                if dim2 > dim1:
                    ...
                    if self.__min_vals[dim2] != self.__max_vals[dim2]:
                        ...
                        x_edges = np.arange(
                                self.__min_vals[dim2] - float(self.__num_bins)/2.0 * self.__bin_widths[dim2],
                                self.__min_vals[dim2]\
                    ...
                    if self.__min_vals[dim1] != self.__max_vals[dim1]:
                        ...
                        y_edges = np.arange(
                                self.__min_vals[dim1] - float(self.__num_bins)/2.0 * self.__bin_widths[dim1],
                                self.__min_vals[dim1]\
                                        + float(self.__num_bins)*self.__bin_widths[dim1],
                                self.__bin_widths[dim1])
        ...
        for dim1 in range(0,self.__ndim):
            for dim2 in range(0,self.__ndim):
                if dim2 > dim1:
                    ...
                    x_edges = np.arange(self.__min_vals[dim2]-self.__bin_widths[dim2],self.__min_vals[dim2] + float(x_num_bins+1)*self.__bin_widths[dim2], self.__bin_widths[dim2])
                    y_edges = np.arange(self.__min_vals[dim1]-self.__bin_widths[dim1],self.__min_vals[dim1] + float(y_num_bins+1)*self.__bin_widths[dim1], self.__bin_widths[dim1])
                    H, x_edges, y_edges = np.histogram2d(x, y, bins=(x_edges,y_edges))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("min_max_desc_cache provides a means to"
    ...
    parser.add_argument('--desc_ext','-de',
            nargs=1,
            default=['.txt'],
            help=desc)
    ...
    parser.add_argument('--consolidate','-c',
                        action="store_true",
                        required=False,
                        default=False,
                        help=desc)
