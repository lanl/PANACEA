...
class DescriptorPlot:
    def __init__(self, **kwargs):
        ...
        self.__all_descriptor_files = panacea.descriptor_file_group.DescriptorFileGroup(
            self.__desc_file_path, self.__desc_file_base, self.__desc_file_ext
        )
    ...
    def plot(self):
        ...
        all_restart_files = panacea.restart_file_group.RestartFileGroup(
            self.__restart_file_path, self.__restart_file_base, self.__restart_file_ext
        )
            ...
            if i >= self.__fig_start and i <= self.__fig_end:
                ...
                if self.__plot_q_dist and all_restart_files[i].entropy_type == "Cross":
                    if all_restart_files[i].primitive_type == "Gaussian":
                        ...
                        norm_vals_q = copy.deepcopy(
                            all_restart_files[i].normalization_coefficients
                        )
                        ...
                        pre_factor = 1.0 / np.sqrt(
                            determin_q * np.power(2.0 * np.pi, float(ndim))
                        )
                        ...
                        for dim1 in range(0, self.__dim_to_plot - 1):
                            for dim2 in range(1, self.__dim_to_plot):
                                if (dim2 - 1) < dim1:
                                    ...
                                    vals_x = np.linspace(
                                        self.__min_vals[dim1] * 0.9,
                                        self.__max_vals[dim1] * 1.1,
                                        self.__grid_res,
                                        endpoint=True,
                                    )
                                    vals_y = np.linspace(
                                        self.__min_vals[dim2] * 0.9,
                                        self.__max_vals[dim2] * 1.1,
                                        self.__grid_res,
                                        endpoint=True,
                                    )
                                    vals_x_norm = (
                                        copy.deepcopy(vals_x) * norm_vals_q[dim1]
                                    )
                                    vals_y_norm = (
                                        copy.deepcopy(vals_y) * norm_vals_q[dim2]
                                    )
                                    ...
                                    for j in range(0, self.__grid_res):
                                        for k in range(0, self.__grid_res):
                                            pt = np.array(
                                                [vals_x_norm[j], vals_y_norm[k]]
                                            )
                                            diff = pt - center_q_new
                                            data[k, j] = pre_factor * np.exp(
                                                -0.5
                                                * np.matmul(
                                                    np.matmul(diff, local_inv_covar),
                                                    diff.transpose(),
                                                )
                                            )
                                    axs[dim1, dim2 - 1].contour(
                                        X_vals, Y_vals, data.transpose(), self.__levels
                                    )
                                ...
                                if (dim2 - 1) < dim1:
                                    ...
                                    for j in range(0, self.__grid_res):
                                        for k in range(0, self.__grid_res):
                                            ...
                                            mu = np.log(
                                                np.divide(
                                                    np.power(center_q_new, 2.0),
                                                    np.sqrt(
                                                        np.power(center_q_new, 2.0)
                                                        + np.power(norm_vals_q, 2.0)
                                                    ),
                                                )
                                            )
                                            pt = np.array(
                ...
                if self.__plot_p_dist and all_restart_files[i].entropy_type == "Self":
                    ...
                    pre_factor = 1.0 / np.sqrt(
                        determin_p * np.power(2.0 * np.pi, float(ndim))
                    )

                    for dim1 in range(0, self.__dim_to_plot - 1):

                        descriptors1 = self.__all_descriptor_files[i].descriptors[
                            :, dim1
                        ]
                        for dim2 in range(1, self.__dim_to_plot):
                            descriptors2 = self.__all_descriptor_files[i].descriptors[
                                :, dim2
                            ]
                            if (dim2 - 1) < dim1:
                                axs[dim1][dim2 - 1].axis("off")
                    ...
                    for dim1 in range(0, self.__dim_to_plot - 1):
                        for dim2 in range(1, self.__dim_to_plot):
                            if (dim2 - 1) < dim1:
                                ...
                                if first_iteration:
                                    descriptors_old = self.__all_descriptor_files[
                                        i
                                    ].descriptors
                                else:
                                    descriptors_old = np.concatenate(
                                        (
                                            descriptors_old,
                                            self.__all_descriptor_files[i].descriptors,
                                        )
                                    )

                                if self.__plot_type == "scatter":
                                    axs[dim1][dim2 - 1].scatter(
                                        descriptors_old[:, dim2],
                                        descriptors_old[:, dim1],
                                        s=0.5,
                                        color="blue",
                                        alpha=0.1,
                                    )
                                else:
                                    descriptors_new_and_old = np.concatenate(
                                        (
                                            descriptors_old,
                                            self.__all_descriptor_files[
                                                i + 1
                                            ].descriptors,
                                        )
                                    )
                                    ...
                                    x_range = (
                                        self.__max_vals[dim2] - self.__min_vals[dim2]
                                    )
                                    y_range = (
                                        self.__max_vals[dim1] - self.__min_vals[dim1]
                                    )
                                    x_num_bins = np.ceil(
                                        float(x_range) / self.__bin_widths[dim2]
                                    )
                                    y_num_bins = np.ceil(
                                        float(y_range) / self.__bin_widths[dim1]
                                    )
                                    x_edges = np.arange(
                                        self.__min_vals[dim2] - self.__bin_widths[dim2],
                                        self.__min_vals[dim2]
                                        + float(x_num_bins) * self.__bin_widths[dim2],
                                        self.__bin_widths[dim2],
                                    )
                                    y_edges = np.arange(
                                        self.__min_vals[dim1] - self.__bin_widths[dim1],
                                        self.__min_vals[dim1]
                                        + float(y_num_bins) * self.__bin_widths[dim1],
                                    ...
                                    if self.__norm_iso_max_values_exist:
                                        levels = np.linspace(
                                            0.0,
                                            self.__norm_iso_max_values[iso_ind],
                                            num=self.__levels,
                                            endpoint=True,
                                        )
                                        axs[dim1][dim2 - 1].contour(
                                            ...
                                            levels=levels,
                                            vmin=0.0,
                                            vmax=self.__norm_iso_max_values[iso_ind],
                                            cmap=plt.get_cmap("nipy_spectral"),
                                        )
                                    else:
                                        axs[dim1][dim2 - 1].contour(
                                            X, Y, H, levels=self.__levels
                                        )

                                axs[dim1][dim2 - 1].set_xlim(
                                    [
                                        self.__min_vals[dim2] * 0.9,
                                        self.__max_vals[dim2] * 1.1,
                                    ]
                                )
                                axs[dim1][dim2 - 1].set_ylim(
                                    [
                                        self.__min_vals[dim1] * 0.9,
                                        self.__max_vals[dim1] * 1.1,
                                    ]
                                )
                                axs[dim1][dim2 - 1].axes.xaxis.set_visible(
                                    self.__ticks_on
                                )
                                axs[dim1][dim2 - 1].axes.yaxis.set_visible(
                                    self.__ticks_on
                                )
                    iso_ind = iso_ind + 1
...
if __name__ == "__main__":
    ...
    desc = (
        "Path to the min max cache directory. By default the folder "
    ...
    desc = (
        "Path to the descriptor file directory. By default will use the "
        "current working directory."
    )
    parser.add_argument("--desc_path", "-dp", nargs=1, default=[os.getcwd()], help=desc)

    desc = (
        "Path to the restart file directory. By default will use the "
        "current working directory."
    )
    parser.add_argument(
        "--restart_path", "-rp", nargs=1, default=[os.getcwd()], help=desc
    )

    desc = (
        "Descriptor files base name.\nE.g. the files should have a base + index +"
    ...
    parser.add_argument("--desc_extension", "-de", nargs=1, default=[".txt"], help=desc)

    desc = (
        "Restart files base name.\nE.g. the files should have a base + index +"
    ...
    parser.add_argument(
        "--restart_base", "-rb", nargs=1, default=["restart"], help=desc
    )

    desc = (
        "Restart files extension.\nE.g. the files should have a base + index +"
        " extension format. For instance restart5.txt can be broken down into "
        "base = restart, index = 5, and extension = .txt\n"
        "By default the extension is assumed to be .ext"
    )
    parser.add_argument(
        "--restart_extension", "-re", nargs=1, default=[".txt"], help=desc
    )

    desc = "Name of the min max cache file. By default minmax.cache is used."
    parser.add_argument(
        "--file", "-f", type=str, nargs=1, default=["minmax.cache"], help=desc
    )
    ...
    parser.add_argument(
        "--levels", "-l", type=int, nargs=1, required=False, default=10, help=desc
    )

    desc = (
        "How to plot the actual data, the default is a scatter plot,"
        " the other option is a contour plot. The z value of the isosurface"
        " is normalized by the number of points in each plot."
    )
    parser.add_argument(
        "--plot_type", "-pt", required=False, default="scatter", help=desc
    )

    desc = "Number of descriptor files to read from current directory. Default is 300"
    parser.add_argument(
        "--num_desc", "-n", type=int, nargs=1, required=False, default=300, help=desc
    )

    desc = "Turn tick marks on."
    parser.add_argument("--ticks_on", "-ton", action="store_true", help=desc)
    ...
    parser.add_argument(
        "--plot_dims", "-d", type=int, nargs=1, required=False, default=5, help=desc
    )

    desc = "Grid resolution of plot. Deafault is 41."
    parser.add_argument(
        "--grid_res", "-g", type=int, nargs=1, required=False, default=41, help=desc
    )

    desc = "Display plots."
    parser.add_argument(
        "--show", "-s", action="store_true", required=False, default=False, help=desc
    )
