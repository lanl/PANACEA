...
std::vector<std::vector<double>> createDistribution(const int samples,
                                                    const int dims) {
...
int main() {
  ...
  std::unique_ptr<BaseDescriptorWrapper> dwrapper =
      panacea_pi.wrap(data, samples, dims);
  ...
  PANACEASettings settings_cross_median =
  ...
  PANACEASettings settings_cross_median_log =
      PANACEASettings::make()
          .set(EntropyType::Cross)
          .set(PANACEAAlgorithm::Flexible)
          .distributionType(kernel)
