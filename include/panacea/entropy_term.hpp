...
public:
  ...
  typedef std::vector<std::any> (*WriteFunction)(
      const settings::FileType file_type, std::ostream &, EntropyTerm *);
...
public:
  /**
   * These are auxillary methods used internally and are not meant to be
   *accessible
   **/
  virtual EntropyTerm::ReadFunction
  getReadFunction(const PassKey<EntropyTerm> &) = 0;
  virtual EntropyTerm::WriteFunction
  getWriteFunction(const PassKey<EntropyTerm> &) = 0;
