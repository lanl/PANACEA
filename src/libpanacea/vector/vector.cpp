...
std::vector<std::any> Vector::write(const settings::FileType file_type,
                                    std::ostream &os,
  ...
  if (file_type == settings::FileType::TXTRestart ||
    ...
    while (line.find("[Vector Type]", 0) == std::string::npos) {
      if (is.peek() == EOF) {
        std::string error_msg =
            "Did not find [Vector Type] header while trying ";
    ...
    int cols;
    {
      ...
      try {
        ...
        std::string error_msg =
            "Unable to read in vector rows from Restart file.\n";
      ...
      try {
        ...
        std::string error_msg =
            "Unable to read in vector cols from Restart file.\n";
        error_msg += "line is: " + line + "\n";
        PANACEA_FAIL(error_msg);
      }
      if (vec.direction() == Direction::AlongRows) {
    ...
    try {
      ...
      std::string error_msg =
          "Error encountered while attempting to read in vector ";
    ...
    std::string error_msg =
        "Vector cannot be read from the specified file type.";
    PANACEA_FAIL(error_msg);
  }
