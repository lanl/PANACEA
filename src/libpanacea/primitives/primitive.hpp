...
public:
  ...
  virtual double compute(const BaseDescriptorWrapper &descriptor_wrapper,
                         ...
                         const settings::EquationSetting &prim_settings =
                             settings::EquationSetting::None) const = 0;

  /*
   * Computes the gradient of the density
   *
   * descriptor_ind - the index of the descriptor value we are interested in
   * calculating the gradient at kernel_ind - the index of the kernel point_ind
   * - the point the gradient is being taken with respect too, (not needed
   * because it must either be the descriptor or the kernel otherwise the
   * gradient is 0.0)
   */
