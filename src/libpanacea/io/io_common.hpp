 ...
 * or false.
 *
 * In particular will look to see if a class T has a postReadInitialization
 *method.
 **/
template <typename, typename T> struct has_post_read_initialization_method {
...
private:
  ...
  static constexpr auto check(T *) ->
      typename std::is_same<decltype(std::declval<T>().postReadInitialization(
                                std::declval<Args>()...)),
                            Ret>::type;
