...
std::unordered_map<std::type_index,
                   FileKernelDistributionTXT::PostReadInitialization>
...
void FileKernelDistributionTXT::write_(std::vector<std::any> &objs,
                                       std::ostream &os) {
...
void FileKernelDistributionTXT::read_(std::any parent,
  ...
  for (auto &obj : objs) {
    ...
    if (read_methods_.count(obj.instance.type()) == 0) {
      std::string error_msg =
          "Unable to read object from restart txt file, read ";
      error_msg += "method is missing.";
      PANACEA_FAIL(error_msg);
    }
...
void FileKernelDistributionTXT::write(std::any obj,
...
void FileKernelDistributionTXT::read(std::any obj,
                                     const std::string &filename) {
  // Not implemented
  std::fstream fs;
