 ...
 **/
enum class CovarianceOption { Flexible, Strict };

// Whether to allocate memory for internal structures or not
enum class CovarianceBuild { Allocate, NoAllocate };
 ...
 *
 * Note, by default covariance matrix creation will adhere to a strict
 * interpretation of what a covariance matrix is. Thus if all of your data
 * points are stacked on top of one another than you will end up with a
 * covariance matrix full of 0.0's which does not make sense and is numerically
 * instable for use with a kernel.
 *
