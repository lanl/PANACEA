...
void updateCovariance(Matrix *covariance, const Vector *current_mean,
  ...
  for (int dim = 0; dim < num_dims; ++dim) {
    { // account for diagonal
      ...
      double A_ij = (*covariance)(dim, dim) *
                        (static_cast<double>(current_num_pts) - 1.0) +
                    static_cast<double>(current_num_pts) *
                        (*current_mean)(dim) * (*current_mean)(dim) +
                    sum;
      double B_ij = static_cast<double>(total_num_pts) * (*new_mean)(dim) *
                    (*new_mean)(dim);

      (*covariance)(dim, dim) =
          1.0 / (static_cast<double>(total_num_pts) - 1.0);
    ...
    for (int dim2 = dim + 1; dim2 < num_dims;
      ...
      double A_ij = (*covariance)(dim, dim2) *
                        (static_cast<double>(current_num_pts) - 1.0) +
                    static_cast<double>(current_num_pts) *
                        (*current_mean)(dim) * (*current_mean)(dim2) +
                    sum;
      double B_ij = static_cast<double>(total_num_pts) * -1.0 *
                    (*new_mean)(dim) * (*new_mean)(dim2);
      (*covariance)(dim, dim2) =
          1.0 / (static_cast<double>(total_num_pts) - 1.0);
...
Covariance::Covariance(const BaseDescriptorWrapper *desc_wrap,
  ...
  correlated::updateCovariance(matrix_.get(), mean_.get(), new_mean.get(),
                               total_number_data_pts_, desc_wrap);
  ...
  if (isZero()) {
    ...
    if (opt == CovarianceOption::Flexible) {
      ...
      std::string error_msg = "Covariance matrix is 0 everywhere, likely "
                              "because your data is stacked.";
    ...
    if (opt == CovarianceOption::Flexible) {
      ...
      std::string error_msg =
          "You are trying to create covariance matrix with a single point";
      error_msg += " Either get a better starting data set or allow "
                   "flexibility in the algorithm.";
      PANACEA_FAIL(error_msg);
    }
...
Covariance::Covariance(std::unique_ptr<Matrix> matrix,
                       std::unique_ptr<Vector> mean, int total_num_pts,
                       const CovarianceOption opt)
    : matrix_(std::move(matrix)), mean_(std::move(mean)),
      total_number_data_pts_(total_num_pts) {

  assert(matrix_->rows() == matrix_->cols() &&
         "Covariance matrix must be square");
  assert(mean_->rows() == matrix_->rows() &&
         "Mean and covariance matrix must have the same dimensions.");
  ...
  for (int i = 0; i < matrix_->rows(); ++i) {
    for (int j = i + 1; j < matrix_->rows(); ++j) {
      ...
      if (std::abs(matrix_->operator()(i, j)) > threshold) {
        ...
        const double diff =
            std::abs(matrix_->operator()(i, j) - matrix_->operator()(j, i));
        const double avg =
            std::abs(matrix_->operator()(i, j) + matrix_->operator()(j, i)) /
            2.0;
        assert(diff / avg < threshold &&
               "Covariance symmetry criteria not satisfied");
      }
    }
  }
...
void Covariance::update(const BaseDescriptorWrapper *desc_wrap) {
  ...
  correlated::updateMean(new_mean.get(), desc_wrap, total_number_data_pts_);
...
int Covariance::rows() const { return matrix_->rows(); }

int Covariance::cols() const { return matrix_->cols(); }
...
double Covariance::getDeterminant() const { return matrix_->getDeterminant(); }
...
double &Covariance::operator()(PassKey<Normalizer>, const int row,
                               const int col) {
  return matrix_->operator()(row, col);
}
...
std::vector<std::any> Covariance::write(const settings::FileType file_type,
  ...
  if (file_type == settings::FileType::TXTRestart ||
    ...
    PANACEA_FAIL("Covariance matrix cannot be written to specified file type "
                 "not supported.");
  }
  return nested_objs;
}
...
io::ReadInstantiateVector Covariance::read(const settings::FileType file_type,
  ...
  if (file_type == settings::FileType::TXTRestart ||
    ...
    while (line.find("[Normalization State]", 0) == std::string::npos) {
      if (is.peek() == EOF) {
        std::string error_msg =
            "Did not find [Normalization State] header while trying ";
        error_msg += "to read restart file.";
        PANACEA_FAIL(error_msg);
      }
      std::getline(is, line);
    ...
    if (line.find("Normalized", 0) != std::string::npos) {
      ...
      std::cout
          << "Warning normalization state value is missing from restart file.";
      std::cout << " Assuming covariance data is unnormalized.\n";
      std::cout << "line is: " << line << std::endl;
      cov_mat->normalized_ = NormalizationState::Unnormalized;
    }

    // Check to see if memory has been allocated to the internal vector and
    // matrix
