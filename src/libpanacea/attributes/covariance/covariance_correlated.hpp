...
#ifndef PANACEA_PRIVATE_COVARIANCE_CORRELATED_H
#define PANACEA_PRIVATE_COVARIANCE_CORRELATED_H
#pragma once
...
namespace panacea {
  ...
  class CovarianceCorrelated : public Covariance {
    private:
    ...
    public:
      CovarianceCorrelated() = default;

      explicit CovarianceCorrelated(const CovarianceBuild );

      CovarianceCorrelated(const BaseDescriptorWrapper & desc_wrap,
          const CovarianceOption opt = CovarianceOption::Strict);

      CovarianceCorrelated(std::unique_ptr<Matrix> matrix,
          std::unique_ptr<Vector> mean,
          int total_num_pts,
          const CovarianceOption opt = CovarianceOption::Strict);

      inline virtual settings::KernelCorrelation correlation() const noexcept final {
...
#endif // PANACEA_PRIVATE_COVARIANCE_CORRELATED_H
