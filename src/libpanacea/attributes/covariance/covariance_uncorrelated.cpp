...
CovarianceUncorrelated::CovarianceUncorrelated(
    const BaseDescriptorWrapper &desc_wrap, const CovarianceOption opt) {
  ...
  covariance::updateCovariance(*matrix_.get(), *mean_.get(), *new_mean.get(),
  ...
  if (isZero()) {
    ...
    if (opt == CovarianceOption::Flexible) {
      ...
      std::string error_msg =
          "You are trying to create covariance matrix with ";
      error_msg +=
          "a single point Either get a better starting data set or allow";
...
CovarianceUncorrelated::CovarianceUncorrelated(std::unique_ptr<Matrix> matrix,
...
void CovarianceUncorrelated::update(const BaseDescriptorWrapper &desc_wrap) {
  ...
  covariance::updateMean(*new_mean.get(), desc_wrap, total_number_data_pts_);
...
int CovarianceUncorrelated::rows() const { return matrix_->rows(); }

int CovarianceUncorrelated::cols() const { return matrix_->cols(); }
...
bool CovarianceUncorrelated::is(const NormalizationState &state) const
    noexcept {
  return normalized_ == state;
}

const NormalizationState &CovarianceUncorrelated::getNormalizationState() const
    noexcept {
  return normalized_;
}

const Matrix &CovarianceUncorrelated::matrix(PassKey<Reducer>) const {
  return *matrix_;
}

void CovarianceUncorrelated::set(PassKey<Normalizer>,
                                 NormalizationState state) {
  normalized_ = state;
}

double &CovarianceUncorrelated::operator()(PassKey<Normalizer>, const int row,
                                           const int col) {
  return matrix_->operator()(row, col);
}

void CovarianceUncorrelated::print() const {
  covariance::printCovariance(*matrix_.get(), *mean_.get(),
                              total_number_data_pts_);
}

std::vector<std::any>
CovarianceUncorrelated::write(const settings::FileType file_type,
                              std::ostream &os, std::any cov_instance) {

  const CovarianceUncorrelated &cov_mat =
...
CovarianceUncorrelated::read(const settings::FileType file_type,
                             std::istream &is, std::any cov_instance) {
