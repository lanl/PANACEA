...
namespace panacea {
  ...
  std::vector<std::any> CovarianceUncorrelated::write(
    ...
    if( file_type == settings::FileType::TXTRestart ||
      ...
      auto cov_mat = dynamic_cast<CovarianceUncorrelated *>(cov_mat_generic);
      ...
      PANACEA_FAIL("CovarianceUncorrelated matrix cannot be written to specified file type not supported.");
    }
    return nested_objs;
  }
  ...
  io::ReadInstantiateVector CovarianceUncorrelated::read(
    ...
    if( file_type == settings::FileType::TXTRestart ||
        file_type == settings::FileType::TXTKernelDistribution ) {
      CovarianceUncorrelated * cov_mat = nullptr;
      if(std::type_index(cov_instance.type()) == std::type_index(typeid(Covariance *))) {
        auto cov_mat_generic = std::any_cast<Covariance *>(cov_instance);
        cov_mat = dynamic_cast<CovarianceUncorrelated *>(cov_mat_generic);
        ...
        cov_mat = dynamic_cast<CovarianceUncorrelated *>(
              cov_mat_generic->get());
      }
