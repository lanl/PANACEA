...
CovarianceCorrelated::CovarianceCorrelated(std::unique_ptr<Matrix> matrix,
...
int CovarianceCorrelated::rows() const { return matrix_->rows(); }

int CovarianceCorrelated::cols() const { return matrix_->cols(); }
...
const NormalizationState &CovarianceCorrelated::getNormalizationState() const
    noexcept {
  return normalized_;
}

const Matrix &CovarianceCorrelated::matrix(PassKey<Reducer>) const {
  return *matrix_;
}
...
double &CovarianceCorrelated::operator()(PassKey<Normalizer>, const int row,
                                         const int col) {
  return matrix_->operator()(row, col);
}
...
std::vector<std::any>
CovarianceCorrelated::write(const settings::FileType file_type,
...
CovarianceCorrelated::read(const settings::FileType file_type, std::istream &is,
