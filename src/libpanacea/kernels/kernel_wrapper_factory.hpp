...
public:
  using KernelCreateMethod = std::unique_ptr<BaseKernelWrapper> (*)(
      const PassKey<KernelWrapperFactory> &, std::any data, const int rows,
      const int cols);
...
private:
  static std::unordered_map<
      ...
      std::unordered_map<std::type_index, // type converting from
                         std::unordered_map<std::type_index, // type converting
                                                             // too
                                            KernelCreateMethod>>>
      create_methods_;
...
public:
  ...
  /**
   * Will create a shell of a kernel wrapper that is essentially empty. This is
   *an ideal state for reading in information from a restart file to populate
   *the kernel wrapper. Note, that the data (Memory) setting must be set to Own
   *to utilize this method.
   **/
  std::unique_ptr<BaseKernelWrapper>
  create(const KernelSpecification &kern_specification) const;

  template <settings::KernelCenterCalculation kernel_center, class T, class U,
            class S>
  static bool registerKernel() {
    if (create_methods_.count(kernel_center) == 0) {
      create_methods_[kernel_center][std::type_index(
          typeid(typename std::remove_const<T>::type))]
                     [std::type_index(typeid(
                         typename std::remove_const<U>::type))] = S::create;
      return true;
    } else {
      if (create_methods_[kernel_center].count(std::type_index(
              typeid(typename std::remove_const<T>::type))) == 0) {
        create_methods_[kernel_center][std::type_index(
            typeid(typename std::remove_const<T>::type))]
                       [std::type_index(typeid(
                           typename std::remove_const<U>::type))] = S::create;
