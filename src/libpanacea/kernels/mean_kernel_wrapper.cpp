...
MeanKernelWrapper::MeanKernelWrapper(const std::vector<double> &mean_vec) {
  if (mean_vec.size() == 0) {
    number_pts_mean_ = 0;
    data_wrapper_ =
        DataPointTemplate<std::vector<double>>(mean_vec, 0, mean_vec.size());
  } else {
    number_pts_mean_ = 1;
    data_wrapper_ =
        DataPointTemplate<std::vector<double>>(mean_vec, 1, mean_vec.size());
  }
}
/************************************************
 * Public Methods
...
int MeanKernelWrapper::rows() const { return data_wrapper_.rows(); }

int MeanKernelWrapper::cols() const { return data_wrapper_.cols(); }

int MeanKernelWrapper::getNumberDimensions() const {
  return data_wrapper_.getNumberDimensions();
}

int MeanKernelWrapper::getNumberPoints() const { return number_pts_mean_; }
...
void MeanKernelWrapper::update(const BaseDescriptorWrapper *dwrapper) {
  ...
  assert(dwrapper->getNumberDimensions() ==
         data_wrapper_.getNumberDimensions());

  Mean mean;
  std::vector<double> new_center =
      mean.calculate<const BaseDescriptorWrapper *, Direction::AlongColumns>(
          dwrapper);
...
void MeanKernelWrapper::print() const { data_wrapper_.print(); }

std::type_index MeanKernelWrapper::getTypeIndex() const noexcept {
  return std::type_index(typeid(std::vector<double>));
}

const settings::KernelCenterCalculation MeanKernelWrapper::center() const
    noexcept {
...
std::ostream &MeanKernelWrapper::write(BaseKernelWrapper *kwrapper_instance,
                                       std::ostream &os) {
  MeanKernelWrapper *kwrapper_mean =
      dynamic_cast<MeanKernelWrapper *>(kwrapper_instance);
