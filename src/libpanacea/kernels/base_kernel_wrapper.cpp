...
namespace panacea {
  ...
  std::vector<std::any> BaseKernelWrapper::read(
      const settings::FileType & file_type,
      std::istream & is,
      std::any kwrapper_instance) {
    ...
    if( file_type == settings::FileType::TXTRestart ) {
      ...
      if( line.find("[Kernel]", 0) == std::string::npos) {
        std::cout << "Warning while reading kernel section of restart file, header";
        std::cout << " does not contain the [Kernel] word.";
      ...
      if( line.find("[Meta Data]", 0) == std::string::npos) {
        std::cout << "Warning while reading kernel section of restart file, header";
        std::cout << " does not contain the [Meta Data] word.";
        std::cout << "line is: " << line  << "\n";
      }
