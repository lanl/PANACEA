...
std::vector<std::any>
BaseKernelWrapper::write(const settings::FileType file_type, std::ostream &os,
...
BaseKernelWrapper::read(const settings::FileType file_type, std::istream &is,
                        std::any kwrapper_instance) {
  ...
  if (file_type == settings::FileType::TXTRestart ||
    ...
    if (std::type_index(kwrapper_instance.type()) ==
      ...
      kwrapper = (std::any_cast<std::unique_ptr<BaseKernelWrapper> *>(
                      kwrapper_instance))
                     ->get();
    }
    assert(kwrapper != nullptr &&
           "Cannot read restart file into null kernel wrapper pointer.");
    // Before doing any reading ensure that the underlying descriptor data type
    // is actually compatible, right now the Kernel must be of type
    // std::vector<std::vector<double>> or std::vector<double> as in the data
    // must be owned by the kernel wrapper and not simply a pointer to it
    if (kwrapper->getTypeIndex() !=
            std::type_index(typeid(std::vector<std::vector<double>>)) &&
        kwrapper->getTypeIndex() !=
            std::type_index(typeid(std::vector<double>))) {
      std::string error_msg =
          "Reading kernel data in requires that the kernel ";
    ...
    while (line.find("[Kernel]", 0) == std::string::npos) {
      if (is.peek() == EOF) {
        std::string error_msg =
            "Warning while reading kernel section of restart file";
    ...
    try {
      ...
      std::string error_msg =
          "Unable to read in kernel center calculation method ";
      error_msg += " from restart file file.\n";
      PANACEA_FAIL(error_msg);
    }
    if (center_calc_method != kwrapper->center()) {
      std::string error_msg =
          "Inconsistency between restart file kernel center ";
      error_msg +=
          "method and the kernel who's state is being loaded from the ";
    ...
    if (kernel_count != kwrapper->count()) {
      std::string error_msg =
          "Inconsistency between restart file kernel count ";
      error_msg +=
          "setting and the kernel who's state is being loaded from the ";
      error_msg += "restart file.";
      PANACEA_FAIL(error_msg);
    }
    ...
    while (line.find("[Meta Data]", 0) == std::string::npos) {
      if (is.peek() == EOF) {
        std::string error_msg =
            "Warning while reading kernel section of restart ";
    ...
    try {
      ...
      std::string error_msg =
          "Error encountered while attempting to read in kernel ";
    ...
    std::string error_msg =
        "Kernels cannot be written to the specified file type.";
    PANACEA_FAIL(error_msg);
  }
