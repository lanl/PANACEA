...
SelfEntropy::getReadFunction(const PassKey<EntropyTerm> &) {
  return SelfEntropy::read;
}

EntropyTerm::WriteFunction
SelfEntropy::getWriteFunction(const PassKey<EntropyTerm> &) {
...
void SelfEntropy::set(const settings::EntropyOption option, std::any val) {
  std::string error_msg =
      "SelfEntropy does not contain any options that can be set";
...
std::vector<std::any> SelfEntropy::write(const settings::FileType file_type,
                                         std::ostream &os,
