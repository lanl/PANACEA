...
SelfEntropy::getReadElements(const PassKey<EntropyTerm> &) {
  return std::vector<EntropyTerm::ReadElement>{
      EntropyTerm::ReadElement{SelfEntropy::read, *this}};
}

std::vector<EntropyTerm::WriteElement>
SelfEntropy::getWriteElements(const PassKey<EntropyTerm> &) const {
  return std::vector<EntropyTerm::WriteElement>{
      EntropyTerm::WriteElement{SelfEntropy::write, *this}};
}
...
SelfEntropy::create(const PassKey<EntropyFactory> &key,
                    const BaseDescriptorWrapper &descriptor_wrapper,
...
void SelfEntropy::update(const BaseDescriptorWrapper &descriptor_wrapper) {
  if (state_ != EntropyTerm::State::Initialized) {
    std::string error_msg =
        "Trying to call update on an entropy term before it has ";
    error_msg +=
        "been initialized, please either initialize the entropy term first";
    error_msg += " or when creating the entropy term provide the descriptors.";
    PANACEA_FAIL(error_msg);
...
std::vector<std::any>
SelfEntropy::write(const settings::FileType file_type, std::ostream &os,
...
SelfEntropy::read(const settings::FileType file_type, std::istream &is,
  ...
  if (file_type == settings::FileType::TXTRestart ||
    ...
    if (entropy_term_instance.type() == settings::EntropyType::Self) {
      try {
        ...
        // This means that only entropy terms that have been initialized should
