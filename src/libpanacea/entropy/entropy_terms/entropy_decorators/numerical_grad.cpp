...
NumericalGrad::getReadElements(const PassKey<EntropyTerm> &key) {
...
NumericalGrad::getWriteElements(const PassKey<EntropyTerm> &) const {
...
std::vector<double> NumericalGrad::compute_grad(
  ...
  if (numerical_grad_) {
    ...
    for (const int &dim : getDimensions()) {
      ...
      descriptor_wrapper(wrt_pt, dim) = new_x_vals.at(0);
      new_y_vals.at(0) +=
          EntropyDecorator::compute(descriptor_wrapper, entropy_settings);
      descriptor_wrapper(wrt_pt, dim) = new_x_vals.at(1);
      new_y_vals.at(1) +=
          EntropyDecorator::compute(descriptor_wrapper, entropy_settings);
      grad.at(dim) = (new_y_vals.at(1) - new_y_vals.at(0)) / (2.0 * diff);
...
std::vector<double> NumericalGrad::compute_grad(
  ...
  return NumericalGrad::compute_grad(const_descriptor_wrapper, wrt_pt,
...
bool NumericalGrad::set(const settings::EntropyOption option, std::any val) {
  if (option == settings::EntropyOption::IncrementRatio) {
    ...
    } else if (std::type_index(val.type()) ==
               std::type_index(typeid(const double))) {
      inc_ratio_ = std::any_cast<const double>(val);
    } else {
      std::string error_msg = "Unsupported type encountered while attempting ";
      error_msg += "to set " + std::string(toString(option)) +
    ...
    if (std::type_index(val.type()) == std::type_index(typeid(bool))) {
      numerical_grad_ = std::any_cast<bool>(val);
               ...
               std::type_index(typeid(const bool))) {
...
std::vector<std::any>
NumericalGrad::write(const settings::FileType file_type, std::ostream &os,
  ...
  if (file_type == settings::FileType::TXTRestart) {
    ...
    try {
      ...
      const NumericalGrad &ent_term =
          dynamic_cast<const NumericalGrad &>(entropy_term_instance);
      os << "[Increment Ratio]\n";
...
NumericalGrad::read(const settings::FileType file_type, std::istream &is,
  ...
  if (file_type == settings::FileType::TXTRestart) {
    ...
    NumericalGrad &ent_term = [&]() -> NumericalGrad & {
      try {
        ...
        std::string error_msg =
            "Unable to cast to NumericalGrad & from EntropyTerm &.";
        PANACEA_FAIL(error_msg);
      }
      return dynamic_cast<NumericalGrad &>(entropy_term_instance);
    }();
    ...
    while (line.find("[Increment Ratio]", 0) == std::string::npos) {
      if (is.peek() == EOF) {
        std::string error_msg =
            "While reading numerical grad decorator section of entropy term ";
        error_msg += "from restart file, file does not contain the [Increment "
                     "Ratio] tag.";
        PANACEA_FAIL(error_msg);
      }
      std::getline(is, line);
