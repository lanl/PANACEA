...
double Weight::compute(const BaseDescriptorWrapper &descriptor_wrapper) {
...
double Weight::compute(const BaseDescriptorWrapper &descriptor_wrapper,
  ...
  return EntropyDecorator::compute(descriptor_wrapper, entropy_settings) *
         weight_;
}
...
double Weight::compute(const BaseDescriptorWrapper &descriptor_wrapper,
                       const int desc_ind,
                       const EntropySettings &entropy_settings) {
  return EntropyDecorator::compute(descriptor_wrapper, desc_ind,
                                   entropy_settings) *
         weight_;
}
...
double Weight::compute(const BaseDescriptorWrapper &descriptor_wrapper,
                       const int desc_ind,
                       const PANACEASettings &panacea_settings) {
  return Weight::compute(descriptor_wrapper, desc_ind,
                         EntropySettings(panacea_settings));
}

std::vector<double>
Weight::compute_grad(const BaseDescriptorWrapper &descriptor_wrapper,
                     const int desc_ind) {

  auto vec = EntropyDecorator::compute_grad(descriptor_wrapper, desc_ind);
  std::transform(
      vec.begin(), vec.end(), vec.begin(),
      std::bind(std::multiplies<double>(), std::placeholders::_1, weight_));
  return vec;
}
...
Weight::compute_grad(const BaseDescriptorWrapper &descriptor_wrapper,
                     const int desc_ind,
                     const PANACEASettings &panacea_settings) {
  return Weight::compute_grad(descriptor_wrapper, desc_ind,
                              EntropySettings(panacea_settings));
}
...
Weight::getReadElements(const PassKey<EntropyTerm> &) {
  std::vector<EntropyTerm::ReadElement> read_functions =
      EntropyDecorator::getReadElements(key);
  read_functions.emplace_back(read, *this);
  return read_functions;
}

std::vector<EntropyTerm::WriteElement>
Weight::getWriteElements(const PassKey<EntropyTerm> &) const {
  std::vector<EntropyTerm::WriteElement> write_functions =
      EntropyDecorator::getWriteElements(key);
  write_functions.emplace_back(write, *this);
  return write_functions;
}

std::vector<std::any> Weight::write(const settings::FileType file_type,
                                    std::ostream &os,
  ...
  if (file_type == settings::FileType::TXTRestart) {
    try {
      const Weight &ent_term =
          dynamic_cast<const Weight &>(entropy_term_instance);
...
io::ReadInstantiateVector Weight::read(const settings::FileType file_type,
                                       std::istream &is,
      ...
      while (line.find("[Weight]", 0) == std::string::npos) {
        if (is.peek() == EOF) {
          std::string error_msg =
              "While reading weight decorator section of entropy term ";
          error_msg +=
              "from restart file, file does not contain the [Weight] tag.";
          PANACEA_FAIL(error_msg);
        }
        std::getline(is, line);
      }
