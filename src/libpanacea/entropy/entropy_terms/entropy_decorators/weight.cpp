...
Weight::getReadFunction(const PassKey<EntropyTerm> &) {
  return read;
}

EntropyTerm::WriteFunction
Weight::getWriteFunction(const PassKey<EntropyTerm> &) {
  return write;
}
...
std::vector<std::any> Weight::write(const settings::FileType file_type,
  ...
  if (file_type == settings::FileType::TXTRestart) {
    ...
    os << "[Weight]\n";
    os << weight->weight_ << "\n";
    nested_values = entropy_term_instance->getWriteFunction(weight->key)(
        file_type, os, entropy_term_instance);
  }
...
io::ReadInstantiateVector Weight::read(const settings::FileType file_type,
  ...
  if (file_type == settings::FileType::TXTRestart) {
    ...
    nested_values = entropy_term_instance->getReadFunction(weight->key)(
        file_type, is, entropy_term_instance);
  }
