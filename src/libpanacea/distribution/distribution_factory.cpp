...
DistributionFactory::DistributionFactory() {
  DistributionFactory::registerDistribution<
      KernelDistribution, settings::DistributionType::Kernel>();
}

std::unique_ptr<Distribution>
DistributionFactory::create(const BaseDescriptorWrapper &descriptor_wrapper,
                            const DistributionSettings &settings) const {

  if (create_methods_.count(settings.type()) == 0) {
    std::string error_msg =
        "Distribution type is not registered with the factory.";
    PANACEA_FAIL(error_msg);
  }

  return create_methods_.at(settings.type())(PassKey<DistributionFactory>(),
                                             descriptor_wrapper, settings);
}

std::unique_ptr<Distribution>
DistributionFactory::create(const DistributionSettings &settings) const {

  return create_shell_methods_.at(settings.type())(
      PassKey<DistributionFactory>(), settings);
}
