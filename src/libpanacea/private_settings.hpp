 ...
 **/
enum class Memory { SelfOwnIfRestartCrossOwn, Own, Share, OwnIfRestart };
...
// Whether we are taking the gradient with respect to the descriptor or the
// kernel
enum class GradSetting { WRTBoth, WRTKernel, WRTDescriptor };

enum class KernelAlgorithm { Strict, Flexible };
 ...
 **/
enum class KernelMemory { Default, Own, OwnIfRestart, Share };

template <class T> constexpr const char *toString(T setting) {
