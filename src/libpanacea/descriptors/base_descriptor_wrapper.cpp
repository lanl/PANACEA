  ...
  std::vector<std::any> BaseDescriptorWrapper::read(
    ...
    if( file_type == settings::FileType::TXTDescriptors ) {
      ...
      if( line.find("Descriptors", 0) == std::string::npos) {
        std::cout << "Warning while reading descriptor file, header does not contain ";
        std::cout << "the Descriptors word, which is typically placed at the top of the";
      ...
      try {
        ...
        std::string error_msg = "Unable to read in columns from Descriptors file.\n";
        error_msg += "line is: " + line + "\n";
        PANACEA_FAIL(error_msg);
      }
      ...
      try { 
        ...
        std::string error_msg = "Error encountered while attempting to read in descriptor ";
