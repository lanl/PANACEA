...
findStackedDimensions(const BaseDescriptorWrapper *desc_wrapper) {
  ...
  for (int dim = 0; dim < desc_wrapper->getNumberDimensions(); ++dim) {
    ...
    // dimension are stacked on top of each other.
    if (diff == 0.0) {
...
static bool printDataQualityMessage1() {
  std::string warning =
      "WARNING you are attempting to calculate normalization ";
  warning +=
      "coefficients using the variance of your data but your data is not ";
  warning +=
      "well dispersed. Your data set is not ideal because you have points";
  warning +=
      " in at least one dimension that are stacked on top of one another ";
  warning +=
      "meaning the variance is 0.0, PANACEA accounts for this by setting ";
  warning +=
      "these particular coefficients to 1.0, however this is not an optimal";
  warning +=
      " solution because it will make it hard for minimizers to optimize ";
  warning +=
      "your configurations it would be better to have data that is more ";
  warning += "varied.";
...
static bool printDataQualityMessage2() {
  ...
  warning +=
      "provided a single point to calculate the variance, which means the ";
  warning +=
      "variance is essentially 0.0. As a work around PANACEA will simply ";
...
normalization_method_variance(const BaseDescriptorWrapper *desc_wrapper,
  ...
  if (extra_args.type() != typeid(settings::None) &&
    ...
    std::string error_msg =
        "Unrecognized types encountered as extra arguments to ";
...
NormalizationMethodFactory::NormalizationMethod
NormalizationMethodFactory::create(
