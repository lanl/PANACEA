 ...
 **/
void trim(const std::vector<double> &in, const NormalizerOption &opt,
...
Normalizer::Normalizer(const settings::KernelNormalization &norm_method,
                       const NormalizerOption opt)
    : norm_option_(opt) {
...
std::vector<std::any> Normalizer::write(const settings::FileType file_type,
                                        std::ostream &os,
...
io::ReadInstantiateVector Normalizer::read(const settings::FileType file_type,
  ...
  if (file_type == settings::FileType::TXTRestart ||
    ...
    while (line.find("[Normalization]", 0) == std::string::npos) {
      if (is.peek() == EOF) {
        std::string error_msg =
            "Missing [Normalization] tag while reading normalization ";
    ...
    try {
      ...
      std::string error_msg =
          "Unable to read in rows from [Normalization] section ";
    ...
    while (line.find("[Normalization Coefficients]", 0) == std::string::npos) {
      if (is.peek() == EOF) {
        std::string error_msg =
            "Missing [Normalization Coefficients] tag while reading";
    ...
    try {
      ...
      std::string error_msg =
          "Error encountered while attempting to read in normalization ";
...
std::istream &operator>>(std::istream &is, NormalizerOption &norm_opt) {
  ...
  if (line.find("Strict", 0) != std::string::npos) {
    ...
    std::string error_msg =
        "Unrecognized normalization option while reading istream.\n";
