 ...
 **/
enum class NormalizerOption { Flexible, Strict };
...
public:
   ...
   **/
  Normalizer(const settings::KernelNormalization &norm_method,
             const NormalizerOption opt = NormalizerOption::Strict);
  ...
  static std::vector<std::any> write(const settings::FileType file_type,
                                     std::ostream &, std::any norm_instance);

  static io::ReadInstantiateVector read(const settings::FileType file_type,
                                        std::istream &, std::any norm_instance);
};

template <class T> inline T Normalizer::get() const noexcept {
  return norm_option_;
}
